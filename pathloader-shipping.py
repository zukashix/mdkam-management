...
try:
    ...
    try:
        ...
        with open('pathloader.txt', 'r') as f:

            APPDATA = f.read()

    except:

        print('FATAL: Error loading pathloader file. Please create a file named \'pathloader.txt\' and write the folder path to use for databases in it. Quitting...')
    ...
    if not os.path.isdir(APPDATA + '/zukashix.mdkam_management'):
        ...
        newUserPWD = pwinput.pwinput(prompt='Enter a password for the new user: ', mask='*')
    ...
    userPassword = pwinput.pwinput(prompt='Please Enter Your Password: ', mask='*')
    ...
    while True:
        ...
        try:
            ...
            if userViewChoice.lower() == 'a':
                ...
                appShipping = 'Univ_Pathloader_Shipping'
    ...
    try:
        ...
        print('FATAL: An unexpected internal error has occurred. Stack trace could not be generated. Please contact the developer.')
        time.sleep(3)
        exit(7)
    ...
    try:
        ...
        print('Stack Trace:')
        print(f'\n{finaltbc}\n')
        print('You may copy this information and contact the developer.')
        time.sleep(3)
        exit(2)
